...
export default class PreMissionScreen extends Component {
                ...
                <Button
                    onPress={() => {
                        if(navigation.state.params.flow === FLOWS.RESUME) {
                            ...
                            Alert.alert(
                                'Create New Sortie?',
                                'By going back, you will be creating a new sortie under the same mission. All data collected for the current sortie will be saved. Do you wish to continue?',
                                [
                                    {
                                        text: 'Yes', onPress: () => {
                                            console.log('OK Pressed');
                                            navigation.navigate("SortieSetup", {
                                                sortieID: navigation.state.params.sortieID,
                                                eventID: navigation.state.params.eventID,
                                                missionID: navigation.state.params.missionID,
    ...
    _onGridCellPress(i,j) {
        ...
        db.transaction((txn) => {
            txn.executeSql("INSERT INTO table_observations (role_id, sortie_id, timestamp, step, event) VALUES(? ,? ,? ,? ,?)",
                [this.navParams.roleIds[roleName], this.navParams.sortieID, new Date().getTime(), STEPS.PRE_MISSION, i], (txn, results) => {
    ...
    _onCancelPressButton() {
        Alert.alert(
            ...
            [
                {text: 'Yes', onPress: () => {
                        console.log('OK Pressed');
                        this.props.navigation.navigate("SortieSetup", {sortieID: this.navParams.sortieID, eventID: this.navParams.eventID, missionID: this.navParams.missionID, flow: FLOWS.BACK_PRESS});
                    }},
                {text: 'No', onPress: () => {
                        console.log('No Pressed');
                    }}
    ...
    _onProceedPressButton() {
        this.props.navigation.navigate("MissionExecution", {gridVals: this.navParams.gridVals, roleIds: this.navParams.roleIds, sortieID: this.navParams.sortieID, eventID: this.navParams.eventID, missionID: this.navParams.missionID});
    }
