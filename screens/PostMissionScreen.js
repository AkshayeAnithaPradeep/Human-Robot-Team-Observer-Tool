...
import {STEPS, FLOWS} from "../apis/values";
...
export default class PostMissionScreen extends Component {
    ...
    static navigationOptions = ({ navigation }) => {
        return {
            ...
            headerLeft: (
                <Button
                    onPress={() => {
                        navigation.navigate("MissionExecution", {sortieID: navigation.state.params.sortieID, eventID: navigation.state.params.eventID, missionID: navigation.state.params.missionID, flow: FLOWS.BACK_PRESS});
                    }}
                    title="Back"
                    color="#fff"
                />
    ...
    _onGridCellPress(i,j) {
        ...
        db.transaction((txn) => {
            txn.executeSql("INSERT INTO table_observations (role_id, sortie_id, timestamp, step, event) VALUES(? ,? ,? ,? ,?)",
                [this.navParams.roleIds[roleName], this.navParams.sortieID, new Date().getTime(), STEPS.POST_MISSION, i], (txn, results) => {
    ...
    _onCancelPressButton() {
        this.props.navigation.navigate("MissionExecution", {sortieID: this.navParams.sortieID, eventID: this.navParams.eventID, missionID: this.navParams.missionID, flow: FLOWS.BACK_PRESS});
    }
    _onProceedPressButton() {
        this.props.navigation.navigate("Summary", {sortieID: this.navParams.sortieID, flow: FLOWS.NEW, eventID: this.navParams.eventID, missionID: this.navParams.missionID});
    }
